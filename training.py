import torch
from model import TrajectoryGCN
...
if __name__ == '__main__':
    ...
    model = TrajectoryGCN(
            input_dim=input_dim,
            hidden_channels=training_wandb_run.config['hidden_channels'],
            output_dim=output_dim,
            dropout=training_wandb_run.config['dropout']
        ).to(device)
    
    optimizer = torch.optim.Adam(model.parameters(), lr=training_wandb_run.config['learning_rate'])
    ...
    for step, batch in enumerate(dataloader):
        ...
        loss = loss_fn(model_outputs, batch.y.to(torch.float32))
