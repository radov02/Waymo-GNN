...
SDC_COLOR = '#00C853'  # Bright green for SDC


def draw_map_features(ax, scenario, x_lim=None, y_lim=None):
    """
    Draw map features (lanes, road edges, crosswalks, etc.) on an axis.
    
    Args:
        ax: Matplotlib axis to draw on
        scenario: Waymo scenario object with map_features
        x_lim: Optional (x_min, x_max) tuple to filter features
        y_lim: Optional (y_min, y_max) tuple to filter features
    
    Returns:
        int: Number of map features drawn
    """
    if scenario is None:
        return 0
    ...
    for feature in scenario.map_features:
        ...
        if feature_type == 'lane' and hasattr(feature.lane, 'polyline'):
            ...
            # Check if any point is within the visible area (if limits provided)
            if x_lim is None or y_lim is None or any(
                x_lim[0] <= x <= x_lim[1] and y_lim[0] <= y <= y_lim[1] 
...
def load_scenario_by_id(scenario_id, scenario_dirs=None, use_index=True, max_files_for_index=None):
    ...
    elif isinstance(scenario_dirs, str):
    ...
    try:
        ...
        print(f"  Warning: Scenario {scenario_id} not found in {scenario_dirs}")
        return None
...
def visualize_training_progress(model, batch_dict, epoch, scenario_id=None, save_dir=viz_training_dir,
    ...
    Creates a B x T grid where rows are graphs and columns are timesteps.
    ...
    # For backward compatibility, set 'scenario' to first loaded scenario
    ...
    for graph_idx in range(B_viz):
        ...
        for t in range(T):
            ...
            # Get the scenario for this specific graph (may differ in batched mode)
            graph_scenario = None
            if scenario_ids_list and graph_idx < len(scenario_ids_list):
                graph_sid = scenario_ids_list[graph_idx]
                if graph_sid in scenarios:
                    graph_scenario = scenarios[graph_sid]
            # Fallback to shared scenario if per-graph not available
            ...
            if graph_scenario is not None and x_lim is not None and y_lim is not None:
                map_features_drawn = 0
                for feature in graph_scenario.map_features:
                    feature_type = feature.WhichOneof('feature_data')
            ...
            for idx, agent_id in enumerate(agent_ids_in_data):
                ...
                for past_t in range(t + 1):  # 0 to current timestep
                    ...
                    if past_data is not None and agent_id in past_data['agent_ids']:
                        ...
                        if not torch.isnan(past_pos).any():
                            historical_positions.append(past_pos.cpu().numpy())
                        else:
                ...
                for future_t in range(t, T):  # current to last displayed timestep
                    ...
                    if future_data is not None and agent_id in future_data['agent_ids']:
                        ...
                        if not torch.isnan(future_pos).any():
                            future_positions.append(future_pos.cpu().numpy())
                        else:
                ...
                curr_xy = curr_xy.cpu().numpy()
                pred_next_xy = pred_next_xy.cpu().numpy()
            ...
            for idx, agent_id in enumerate(agent_ids_in_data):
                ...
                curr_xy = curr_xy.cpu().numpy()
                actual_next_xy = actual_next_xy.cpu().numpy()
                pred_next_xy = pred_next_xy.cpu().numpy()
            ...
            for idx, agent_id in enumerate(agent_ids_in_data):
                ...
                if torch.isnan(curr_xy).any():
                    continue
                actual_next_xy = actual_next_pos[idx].cpu().numpy()
                pred_next_xy = pred_next_pos[idx].cpu().numpy()
