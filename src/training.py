...
def directional_loss(pred, target, alpha=0.7):
    """
    Combined loss with both magnitude (MSE) and direction (cosine similarity).
    
    Args:
        pred: Predicted displacements [N, 2]
        target: Ground truth displacements [N, 2]
        alpha: Weight for MSE component (1-alpha for angular component)
    
    Returns:
        Combined loss that penalizes both magnitude and direction errors
    """
    # MSE loss for magnitude
    mse_loss = F.mse_loss(pred, target)
    
    # Normalize vectors to unit length for direction comparison
    # Add small epsilon to avoid division by zero for stationary agents
    ...
    # cos_sim ranges from -1 to 1, where 1 means same direction
    # We want to minimize (1 - cos_sim), which ranges from 0 to 2
    cos_sim = F.cosine_similarity(pred_norm, target_norm, dim=1)
    angular_loss = (1 - cos_sim).mean()
    
    # Combined loss
    return alpha * mse_loss + (1 - alpha) * angular_loss
